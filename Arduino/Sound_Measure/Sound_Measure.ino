...
//For help and reference https://blog.yavilevich.com/2016/08/arduino-sound-level-meter-and-spectrum-analyzer/
//Download and import Arduino FHT library 4 from http://wiki.openmusiclabs.com/wiki/ArduinoFHT
...
#include <FHT.h>
float FinalAnalog1= 0;
float FinalVolume1= 0;
float FinalFrequency1= 0;
...
void MeasureVolume()
{
  ...
  //sei();
  float dB=0.000;
  if (soundVolRMSflt<1.000){
    dB= 0.000;
  }
  else{
    dB = 20.0*log10(soundVolRMSflt/1.000);
  }
...
void MeasureFHT()
{
  ...
  { 
    if (FreqOutData[i]>Fmax) {
    Fmax= FreqOutData[i];
    maxIndex= i;
    float RelevantBinMax = 150*(i+1);
    FinalFrequency1 = RelevantBinMax;
   // Serial.println("Frequency" + String (RelevantBinMax));
...
Serial.println(String(FinalVolume1,3) + "," + String (FinalFrequency1));
